...
public:
  ...
  public:
    ...
    bool isAdjacentTo(const Vertex& v) const {
        //IncidenceEdgesItor t1 = v_obj->elt;
        //IncidenceEdgesItor t2 = v->elt;
        EdgeList& el1 = v_obj->incidentEdges();
        EdgeList& el2 = v.incidentEdges();
        for(EdgeItor it = el1.begin(); it != el1.end(); it++){
            if()
        }
        for(EdgeItor it = el2.begin(); it != el2.end(); it++){
            if()
        }
        return false;
    }
