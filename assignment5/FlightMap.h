...
public:
   ...
   * a simple path that contains on cycle. The route does not have
...
private:
  ...
  bool findRoute(const FlightGraph::Vertex &v1, const FlightGraph::Vertex &v2, FlightGraph::VertexList &visitedVertices);

  void findReachableAirports(const FlightGraph::Vertex &v, FlightGraph::VertexList &visitedVertices);

  vector<int> findAllShortestRoutes(const FlightGraph::Vertex &vertex_origin);

  double calculateDistanceBetweenVertices(const FlightGraph::Vertex &vertex_origin, const FlightGraph::Vertex &vertex_dest);

  bool isInVisited(const FlightGraph::Vertex &v, FlightGraph::VertexList &visitedVertices);
